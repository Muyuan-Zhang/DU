...
from cacti.utils.scheduler_builder import build_scheduler
from cacti.utils.utils import load_checkpoints, get_device_info, get_exp_weights, wait_for_gpu
...
def loss_f(loss_func, pred, lbl, eps=1e-8):
    return torch.sqrt(torch.mean((pred - lbl) ** 2) + eps)



if __name__ == "__main__":
    wait_for_gpu(gpu_index=0, min_free_gb=1, check_interval=60)
    ...
    log_dir = log_dir + f'/{cfg.model.type}'
    ...
    if cfg.resume is None:
        ...
        logger = Logger(log_dir, cfg.model.type)
    ...
    mask_tensor = torch.from_numpy(mask).float().to(args.device)  # [C,H,W]
    mask_s_tensor = torch.from_numpy(mask_s).float().to(args.device)  # [C,H,W] or [H,W]
    # video_sci dataset
    ...
    for epoch in range(start_epoch, cfg.runner.max_epochs):
        ...
        for iteration, data in enumerate(train_data_loader):
            ...
            Phi = mask_tensor.unsqueeze(0).expand(batch_size, -1, -1, -1)  # [B,C,H,W]
            Phi_s = mask_s_tensor.unsqueeze(0).unsqueeze(1).expand(batch_size, -1, -1, -1)
            if cfg.amp:
                with autocast(device_type="cuda", dtype=torch.float16):
                    model_out = model(meas, (Phi, Phi_s))

                    if cfg.multi_stage_loss:
                        weights = get_exp_weights(cfg.model.stage, beta=cfg.beta, device=gt.device)
                        model_loss = sum(w * loss_f(mse, model_out[-i], gt) for i, w in enumerate(weights, 1))
                    else:
                        model_loss = loss_f(mse, model_out[-1], gt)
                ...
                if cfg.multi_stage_loss:
                    ...
                    for i, w in enumerate(weights, 1):
                        model_loss += w * loss_f(mse, model_out[-i], gt)
                else:
                    model_loss = loss_f(mse, model_out[-1], gt)

                loss = model_loss / accum_iter

            scaler.scale(loss).backward()
            epoch_loss += loss.item() * accum_iter

            if (iteration + 1) % accum_iter == 0 or (iteration + 1) == len(train_data_loader):
