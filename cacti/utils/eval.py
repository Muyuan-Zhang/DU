...
def eval_psnr_ssim(model, test_data, mask, mask_s, args, model2=None):
    psnr_dict, ssim_dict = {}, {}
    psnr_list, ssim_list = [], []
    out_list, gt_list = [], []
    ...
    for iter, data in enumerate(data_loader):
        ...
        for ii in range(batch_size):
            ...
            if model2 is not None:
                with torch.no_grad():
                    refined = model2(single_meas, output, (Phi, Phi_s))
                    output = refined
            # print("Measurement consistency output shape:", output.shape)
            # print("Measurement consistency gt shape:", gt.shape)
