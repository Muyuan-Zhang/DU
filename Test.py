...
if __name__ == "__main__":
    ...
    Phi = torch.from_numpy(Phi).to(args.device).contiguous()
    Phi_s = torch.from_numpy(Phi_s).to(args.device).contiguous()
    ...
    for data_iter, data in enumerate(data_loader):
        ...
        for ii in range(batch_size):
            single_gt = gt[ii]
            single_meas = meas[ii].unsqueeze(0).unsqueeze(0).contiguous()
            if "partition" in cfg.test_data.keys():
            ...
            with torch.no_grad():
                ...
                if ii > 0:
                    sum_time += run_time
                    time_count += 1
            output = outputs[-1][0].clip(0, 1).cpu().numpy().astype(np.float32)
            ...
            for jj in range(cr):
                ...
                # psnr += ski_psnr(per_frame_gt, per_frame_out, data_range=1)
                # ssim += ski_ssim(per_frame_gt, per_frame_out, data_range=1)
